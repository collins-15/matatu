		...
		$.ajax({
					...
					if (typeof (resp) != undefined) {
						...
						if (Object.keys(resp).length > 0) {
							...
							Object.keys(resp).map(k => {
								...
								tr.append('<td class="">' + resp[k].first_name + '</td>')
								tr.append('<td class="">' + resp[k].booked_seat + '</td>')
							...
							$('#booked-field tbody').html('<tr><td colspan="7" class="text-center">No result.</td></tr>')
						}
					}
				}
			},
			complete: function () {
				...
				$('.edit_booked').click(function () {
					uni_modal('Edit Booked', 'book/customer_book.php?id=' + $(this).attr('data-id') + '&bid=' + $(this).attr('data-bid'), 1)
				})
			}
		})
	}
